...
async function sendOwnerLoginAlertEmail(toEmail, name = "") {
  const greeting = name && name.trim() ? `Hello ${name.trim()},` : "Hello Fleet Owner,";
  const loginTime = getFormattedDateTime();
  ...
  const response = await fetch("https://api.brevo.com/v3/smtp/email", {
    ...
    body: JSON.stringify({
      sender: { name: SENDER_NAME, email: SENDER_EMAIL },
      to: [{ email: toEmail }],
      subject: "Security Alert: Login to your MyCabExpress Fleet Owner Portal",
      htmlContent: `
        <div style="font-family: Arial, sans-serif; max-width: 440px; margin: auto; padding: 24px; border: 1px solid #E2E8F0; border-radius: 16px;">
          <div style="text-align:center; margin-bottom: 20px;">
            <h2 style="color:#000428; margin: 0 0 6px 0; font-size: 22px; font-weight: 800;">MyCabExpress</h2>
            <p style="color:#004e92; font-size: 13px; margin: 0; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Fleet Owner Portal</p>
          </div>
          <p style="color:#334155; font-size: 15px; margin-bottom: 12px;">
            ${greeting}
          </p>
          <p style="color:#4A505C; font-size: 14px; line-height: 20px;">
            Your MyCabExpress Fleet Owner account was just logged in.
          </p>
          <div style="background:#F0FDF4; border: 1px solid #BBF7D0; border-radius:12px; padding:16px; margin:18px 0;">
            <p style="margin:0 0 6px 0; font-size:13px; color:#166534;"><strong>Time:</strong> ${loginTime} (IST)</p>
            <p style="margin:0 0 6px 0; font-size:13px; color:#166534;"><strong>Application:</strong> MyCabExpress Android Owner App</p>
            <p style="margin:0; font-size:13px; color:#166534;"><strong>Role:</strong> Fleet Manager / Owner</p>
          </div>
          <p style="color:#4A505C; font-size:13px; line-height: 18px;">
            If this was you, no action is needed. If you did not log in, please reset your password immediately via the Owner App login screen to protect your fleet.
          </p>
          <hr style="border: none; border-top: 1px solid #E2E8F0; margin: 20px 0 12px 0;" />
          <p style="color:#94A3B8; font-size:11px; margin:0; text-align:center;">
            MyCabExpress Fleet Operations · Automated Dispatch System
          </p>
        </div>
      `,
    }),
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}));
    console.log("Brevo owner login alert error:", errorData);
  }
}

async function sendOwnerLogoutAlertEmail(toEmail, name = "") {
  const greeting = name && name.trim() ? `Hello ${name.trim()},` : "Hello Fleet Owner,";
  const logoutTime = getFormattedDateTime();
  ...
  const response = await fetch("https://api.brevo.com/v3/smtp/email", {
    ...
    body: JSON.stringify({
      sender: { name: SENDER_NAME, email: SENDER_EMAIL },
      to: [{ email: toEmail }],
      subject: "Security Alert: Logged out from your MyCabExpress Fleet Owner Portal",
      htmlContent: `
        <div style="font-family: Arial, sans-serif; max-width: 440px; margin: auto; padding: 24px; border: 1px solid #E2E8F0; border-radius: 16px;">
          <div style="text-align:center; margin-bottom: 20px;">
            <h2 style="color:#000428; margin: 0 0 6px 0; font-size: 22px; font-weight: 800;">MyCabExpress</h2>
          ...
          <p style="color:#4A505C; font-size: 14px; line-height: 20px;">
            You have been successfully logged out from the MyCabExpress Fleet Owner App.
          </p>
          <div style="background:#F1F5F9; border: 1px solid #E2E8F0; border-radius:12px; padding:16px; margin:18px 0;">
            <p style="margin:0 0 6px 0; font-size:13px; color:#334155;"><strong>Time:</strong> ${logoutTime} (IST)</p>
            <p style="margin:0; font-size:13px; color:#334155;"><strong>Session:</strong> Terminated safely</p>
          </div>
          <p style="color:#4A505C; font-size:13px; line-height: 18px;">
            You can log back into your fleet dashboard anytime using your registered email and password.
          </p>
          <hr style="border: none; border-top: 1px solid #E2E8F0; margin: 20px 0 12px 0;" />
          <p style="color:#94A3B8; font-size:11px; margin:0; text-align:center;">
            MyCabExpress Fleet Operations · Automated Dispatch System
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}));
    console.log("Brevo owner logout alert error:", errorData);
  }
}
